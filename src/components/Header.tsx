    ...
    <header className="site-header sticky top-0 z-[80] backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
        ...
        <nav className="hidden items-center gap-1 text-sm font-semibold md:flex">
          <Link href="/" className={`rounded-lg px-4 py-2 hover:text-brand-600 ${pathname === "/" ? "text-brand-600" : ""}`}>Home</Link>
          <Link href="/articles" className={`rounded-lg px-4 py-2 hover:text-brand-600 ${pathname.startsWith("/articles") ? "text-brand-600" : ""}`}>Articles</Link>
          <Link href="/about" className={`rounded-lg px-4 py-2 hover:text-brand-600 ${pathname.startsWith("/about") ? "text-brand-600" : ""}`}>About</Link>
          <Link href="/contact" className={`rounded-lg px-4 py-2 hover:text-brand-600 ${pathname.startsWith("/contact") ? "text-brand-600" : ""}`}>Contact</Link>
          <ThemeToggle />
        </nav>
        <div className="flex items-center gap-1 md:hidden">
          <ThemeToggle />
          <button className="relative z-[90] rounded-lg p-2" onClick={() => setOpen((v) => !v)} type="button" aria-label={open ? "Close menu" : "Open menu"}>
      ...
      {open && (
        <div className="fixed inset-0 z-[70] md:hidden">
          ...
          <div className="card absolute right-3 top-[62px] w-[min(280px,calc(100%-1.5rem))] p-3 shadow-xl">
