    ...
    <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4">
        <Link href="/" className="flex items-center gap-3">
          <img src="/logo.svg" alt="Global Career Hub" className="h-11 w-11 rounded-xl border border-dark-800" />
          <div>
            ...
            <p className="mt-1 text-[10px] font-bold uppercase tracking-widest text-slate-500">Abdul Faheem</p>
          </div>
        </Link>
        <nav className="hidden items-center gap-1 text-sm font-semibold text-slate-700 md:flex">
          ...
          <Link href="/contact" className={`rounded-lg px-4 py-2 hover:bg-slate-100 hover:text-brand-600 ${pathname.startsWith("/contact") ? "text-brand-600" : ""}`}>Contact</Link>
        </nav>
      ...
      {open && (
        <div className="space-y-2 border-t border-slate-200 bg-white px-4 py-4 md:hidden">
          ...
          <Link href="/contact" className="block rounded-lg px-3 py-2" onClick={() => setOpen(false)}>Contact</Link>
        </div>
      )}
    </header>
  );
