...
export default async function PublisherHome() {
  ...
  return (
    <div className="min-h-screen px-4 py-8 pb-24" style={{ background: "var(--bg)", color: "var(--fg)" }}>
      <div className="mx-auto max-w-3xl space-y-6">
        <div className="card p-6">
          ...
          <p className="mt-2 text-sm" style={{ color: "var(--muted)" }}>You can write drafts for Global Career Hub. An admin reviews the site.</p>
        </div>
        ...
        <div className="card p-2">
          {posts.length === 0 ? <p className="p-6 text-sm" style={{ color: "var(--muted)" }}>No drafts yet.</p> : (
            <ul>
              {posts.map((p) => (
                <li key={p.id} className="flex items-center justify-between border-b px-4 py-3 last:border-0" style={{ borderColor: "var(--border)" }}>
                  <div>
                    <p className="font-semibold">{p.title}</p>
                    <p className="text-xs capitalize" style={{ color: "var(--muted)" }}>{p.status}</p>
                  </div>
      ...
      <nav className="site-header fixed bottom-0 left-0 right-0 grid grid-cols-4 text-center text-[11px] font-bold uppercase">
