...
export default async function HomePage() {
  ...
  return (
    <>
      <section className="relative overflow-hidden border-b py-20 lg:py-28" style={{ borderColor: "var(--border)", background: "var(--bg)", color: "var(--fg)" }}>
        <div className="absolute inset-0 bg-[radial-gradient(#e11d48_1px,transparent_1px)] bg-[size:24px_24px] opacity-15" />
        <div className="relative z-10 mx-auto max-w-3xl px-4 text-center">
          <p className="inline-flex items-center gap-2 rounded-full border px-3.5 py-1.5 text-xs font-bold uppercase tracking-wider text-brand-500" style={{ borderColor: "var(--border)", background: "var(--bg2)" }}>Career, skills and study guides</p>
          ...
          <p className="mx-auto mt-5 max-w-2xl text-base" style={{ color: "var(--muted)" }}>High-value writing on work, tech, scholarships, and study paths. Read completely free.</p>
          <form action="/search" className="mx-auto mt-8 flex max-w-xl items-center rounded-2xl border p-2" style={{ borderColor: "var(--border)", background: "var(--bg2)" }}>
            <input name="q" placeholder="Search guides..." className="w-full bg-transparent px-3 text-sm outline-none" style={{ color: "var(--fg)" }} />
            <button className="btn !rounded-xl" type="submit">Search</button>
          </form>
        </div>
      </section>
      <section className="mx-auto max-w-7xl px-4 py-16">
        <div className="mb-10 flex items-end justify-between border-b pb-4" style={{ borderColor: "var(--border)" }}>
          <div>
        ...
        {posts.length === 0 ? (
          <div className="card p-8"><p className="font-heading text-xl font-bold">New guides are on the way</p><p className="mt-2 text-sm" style={{ color: "var(--muted)" }}>Browse search topics or check back soon for new guides.</p></div>
        ) : (
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">{posts.map((post) => <PostCard key={post.id} post={post} />)}</div>
        )}
      </section>
      <section className="border-y py-16" style={{ borderColor: "var(--border)", background: "var(--bg2)", color: "var(--fg)" }}>
        <div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-8 px-4 md:flex-row md:items-center">
          <div className="max-w-2xl">
            ...
            <p className="mt-3 text-sm" style={{ color: "var(--muted)" }}>Guides on careers, skills, and study routes.</p>
          </div>
